...
void CalculatorWidget::valueChanged()
{
    ...
    bool okFirst;
    int firstValue = firstValueText.toInt(&okFirst);

    QString secondValueText = m_secondValue->text();
    bool okSecond;
    int secondValue = secondValueText.toInt(&okSecond);

    if(!okFirst || !okSecond) { m_result->setText("no result"); }
    else { m_result->setText(QString::number(firstValue + secondValue)); }
}
